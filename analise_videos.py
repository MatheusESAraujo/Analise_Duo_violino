...
def analise_camera(camera=0,tamanho=800,param_lim=0.5):
...
def analise_video_imshow(local_video,tamanho=800,param_lim=0.5):
...
def analise_video(local_video,tamanho=800,param_lim=0.5):
    cap = cv2.VideoCapture(local_video)
    length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    print( length )
